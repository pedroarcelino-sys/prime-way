...
/* Smoke test somente leitura do painel administrativo. */

final class PrimewayDashboardSessaoMemoria implements SessionHandlerInterface
{
...
session_set_save_handler(new PrimewayDashboardSessaoMemoria(), true);
primewayIniciarSessao();

$pdo = primewayPdo();
$usuarioId = (int) $pdo->query(
    "SELECT id FROM usuarios WHERE perfil = 'admin' AND ativo = 1 ORDER BY id LIMIT 1"
)->fetchColumn();
...
require $raiz . '/api/dashboard/index.php';
